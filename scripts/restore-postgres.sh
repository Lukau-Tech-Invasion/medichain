...
# read n_live_tup after an ANALYZE while the backup read it WITHOUT one, which
# meant a data-free restore compared equal to a manifest of zeros and was
# reported as verified (Horizon HZ-027). No ANALYZE is needed now, because
# nothing here depends on planner statistics.
# shellcheck source=scripts/lib/row-count-query.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib/row-count-query.sh"
docker exec "$CONTAINER" psql -U "$DB_USER" -d "$TARGET_DB" -t -A \
  -c "$(row_count_query "$EXCLUDE_SCHEMA_REGEX")" > "$ACTUAL_FILE"
