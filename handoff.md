...
| Note-template registry and rendering | partially implemented, verified | The portal maps the server registry’s actual snake-case/content shape into renderable sections and can request/display a rendered draft; focused page tests pass (5/5). The server renders structured template content with explicit substitutions, retains the full admission-template section set, and rejects unknown ids; focused Rust tests pass (3/3). Creating, duplicating, and deleting templates are still local-only UI controls and need an authorized persistence design. |
