...
**What's needed:**
- [ ] **Dependency audit: confirm the node crate is on a maintained dependency line.** Parity merged the standalone `substrate`/`polkadot`/`cumulus` repos into the unified `polkadot-sdk` monorepo in 2023–2024; the old standalone `paritytech/substrate` repo is no longer where active development happens. Verify `node/Cargo.toml`'s `sc-*`/`sp-*` crate versions are pulling from a still-patched line (not an orphaned pre-merger release) before going anywhere near production — this is a real security-patching risk, not a style preference.
