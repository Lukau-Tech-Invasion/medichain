...
| AUTH-003 | P2 | The intended JWT-session contract is not yet the sole client/server identity mechanism: many portal pages and test/demo scripts still emit legacy `X-User-Id`. | `api/src/support.rs`, `api/src/middleware/signature_auth.rs`, `client/shared/src/api/client.ts`, direct portal fetch call sites, test/demo scripts | Source review found production signature middleware rejects an `X-User-Id` request without its bound wallet signature, while a valid Bearer token resolves identity from its verified `sub` claim. The typed shared client now omits `X-User-Id` whenever it has a Bearer token, retaining header-only behavior only for tokenless demo compatibility. A reusable `getSessionHeaders()` helper migrates the clinician and patient dashboards. Shared, clinician, and patient TypeScript checks pass; focused dashboard tests pass (clinician 4/4, patient 3/3). The initial direct-portal inventory was 104 occurrences across 54 files (79 clinician, 25 patient); 101 direct-portal occurrences remain (78 clinician, 23 patient). Shared-client and documentation references are counted separately. | No current production-mode browser session, JWT-only role workflow, or negative proof that every protected route rejects the legacy header after migration. | The legacy header remains deliberately allowed in demo mode, and a broad direct-fetch migration must preserve wallet-signature step-up where policy requires it. The normal typed-client path is improved, but the full requested migration is incomplete. | pending | PARTIALLY FIXED |
