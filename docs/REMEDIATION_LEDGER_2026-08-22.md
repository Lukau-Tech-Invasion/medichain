...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SEC-001 | P1 | Anonymous routes disclosed wallet-linked identity before ownership proof and had only a process-local request throttle. | `api/src/auth_challenges.rs`, `api/migrations/20260821000001_auth_challenges.sql`, auth handlers/routes/types, and shared/portal auth clients | Focused challenge and JWT-field tests pass. `a65f19f` adds a PostgreSQL advisory-lock-protected per-wallet rolling budget of five challenges per minute; its six-way concurrent test passed with exactly five issues and one typed rate-limit result. | Earlier freshly rebuilt local image returned matching non-identifying challenge shapes for two wallet addresses. The new durable throttle has compile and database evidence but has not yet been rebuilt and exercised through the public endpoint. | Browser wallet signing, valid proof, replay, expiry, and broader database race verification not yet run. | `8a7a5e3`, `a65f19f` | PARTIALLY FIXED |
