...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | All patient access request/grant transitions now span their repository and audit event. Guardian revocation and permission updates now do; emergency grant authority is PostgreSQL-backed but its audit writes remain separate. Nine mapped fail-open flows remain. | Isolated PostgreSQL duplicate-event tests prove every patient-access transition rolls back; approval/denial/revocation normal paths also commit with their event. Guardian revocation and permission update each now have rollback proof (`1 passed`, 430/431 filtered). | `cargo check -p medichain-api --message-format short` passes. Source inspection still identifies fail-open audit branches in emergency access/grants, guardian create, device lifecycle, identity claims, and mobile records. | Guardian revocation/permission updates have direct rollback proof but no fresh normal-path event tests. Emergency authority has restart/binding/revocation database proof, but not atomic audit proof. This remains a release blocker. | pending | PARTIALLY FIXED |
