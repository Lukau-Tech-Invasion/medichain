...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | Transactional access-request creation, approval, **and denial** now span `api/src/handlers/access_control.rs`, `patient_access`, and both repository implementations; revoke and 11 other mapped high-risk flows remain. | Isolated PostgreSQL fault-injection tests reserve outbox event IDs for creation, approval, and denial. Each passes only when its business transition rolls back; approval additionally has a separate success-path test confirming the request, grant, and event commit. | `cargo check -p medichain-api --message-format short` passes. Source inspection still identifies fail-open `audit outbox write failed` branches in emergency access/grants, RBAC, device lifecycle, identity claims, mobile records, and the access-control revoke transition. | Creation, approval, and denial have direct database rollback proof. Only approval has fresh success-path proof; denial success, revoke, restart/browser, and remaining high-risk flows remain unverified. This remains a release blocker. | pending | PARTIALLY FIXED |
