...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | Transactional access-request creation now spans `api/src/handlers/access_control.rs`, `patient_access`, and both repository implementations; 13 other mapped high-risk flows remain. | New isolated PostgreSQL fault-injection test reserves an outbox event ID, forces its duplicate insert during access-request creation, and passes only when the business row is rolled back (`1 passed`, 422 filtered). | API compilation passes. Source inspection still identifies fail-open `audit outbox write failed` branches in emergency access/grants, RBAC, device lifecycle, identity claims, mobile records, and some access-control transitions. | The access-request creation slice has direct database rollback proof. No restart/browser proof and no atomic coupling proof for the remaining high-risk flows. This remains a release blocker. | pending | PARTIALLY FIXED |
