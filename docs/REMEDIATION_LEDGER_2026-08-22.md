...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | Mapped 14 handler call sites across access control, emergency access/grants, RBAC, device lifecycle, identity claims, mobile records, and platform registries; `api/src/audit_outbox.rs` | Existing durability test proves `record_durable()` persists an event when invoked; it does not prove atomic coupling with the corresponding business write. | Source inspection confirms nine handler modules contain an `audit outbox write failed` branch that continues the successful operation. | No fault-injection, transaction-rollback, restart, or browser proof. This is a still-open release blocker, not closed by durable-event persistence alone. | — | STILL PRESENT |
...
| TEST-002 | P2 | The clinician frontend full-test gate is not currently reliable under its configured 10-second timeout; its full suite also emits framework compatibility warnings. | `client/doctor-portal/src/store/credentialKeystore.test.ts`, Vitest configuration, affected page rendering keys | Patient full suite passed: 26 files, 82 tests. Clinician full suite failed: 82 files passed, 303 tests passed, one `credentialKeystore` seed-derived-account round-trip test exceeded the configured 10,000 ms timeout. Duplicate-key handling was corrected for provider options and note-template/section lists; focused `AppointmentsPage` (7 tests) and `NoteTemplatesPage` (3 tests, single worker) now pass without their former key warnings. React Router v7 future warnings remain across both suites. | No browser mutation or production build proof for these cases. | Unit tests are not browser evidence. The keystore test is security-relevant because it covers a clinician signing credential. | pending | PARTIALLY FIXED |
