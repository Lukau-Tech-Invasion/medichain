...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts were often separate operations; handlers could log an outbox failure and still return a successful mutation. | Patient access, guardian relationships, and PostgreSQL-backed emergency grant issue/revocation now span their business transition and prepared audit event. Four mapped fail-open branches remain: device lifecycle, identity claims, mobile records, and registry bulk-read auditing. | Isolated PostgreSQL tests prove patient-access and guardian rollback behavior. Emergency grant issuance commits with its durable event; forced audit-insert failures roll back both issuance and revocation. | `cargo check -p medichain-api --message-format short` passes. The static inventory no longer finds emergency access/grant fail-open audit branches. | Emergency authority has restart/binding/revocation database proof and now transactional audit proof. No browser emergency workflow, deployed-image, or remaining-flow atomicity evidence exists. This remains a release blocker. | pending | PARTIALLY FIXED |
