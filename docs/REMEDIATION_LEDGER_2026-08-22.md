...
| AUTH-003 | P2 | The intended JWT-session contract is not yet the sole client/server identity mechanism: the shared client prefers Bearer JWT, but many portal pages and test/demo scripts still emit legacy `X-User-Id`. | `api/src/support.rs`, `api/src/middleware/signature_auth.rs`, `client/shared/src/api/client.ts`, direct portal fetch call sites, test/demo scripts | Source review found production signature middleware rejects an `X-User-Id` request without its bound wallet signature, while a valid Bearer token resolves identity from its verified `sub` claim. It also found direct portal calls that still construct `X-User-Id` headers rather than using the shared JWT client. | No current production-mode browser session, JWT-only role workflow, or negative proof that every protected route rejects the legacy header after migration. | The legacy header remains deliberately allowed in demo mode, and a broad client migration must preserve wallet-signature step-up where policy requires it. This is not complete merely because unsigned production writes are rejected. | pending | STILL PRESENT |
