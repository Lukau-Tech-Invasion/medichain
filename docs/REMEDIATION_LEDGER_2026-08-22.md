...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SEC-001 | P1 | Anonymous routes disclosed wallet-linked identity before ownership proof. | `api/src/auth_challenges.rs`, `api/migrations/20260821000001_auth_challenges.sql`, auth handlers/routes/types, shared and portal auth clients | Focused challenge and JWT-field tests pass. | Freshly rebuilt local image manifest `sha256:926596…` is healthy. Gateway requests for two wallet addresses both returned `200` with exactly `challenge`, `instructions`, and `success`, and neither response contained name, role, patient, linkage, email, or username fields. The local image label is `local-unverified`, so this is not release-provenance evidence. | Browser wallet signing, valid proof, replay, expiry, and database race verification not yet run. | `8a7a5e3` | PARTIALLY FIXED |
...
| PRIV-001 | P1 | Sensitive identifiers can enter logs and related telemetry. | `api/src/privacy_logging.rs`, logging initialization, `api/src/middleware/signature_auth.rs`, `api/src/main.rs`, and 18 production call-site files | Sanitizer and `log::Record` sink-path leakage tests pass, including labelled wallet fields. `ad1ad6c` removes direct wallet, patient, user, and record-ID interpolation from staff login, MFA, emergency, FHIR, surgical, messaging, retention, and related paths; `cargo check --bin medichain-api` passes. | Earlier isolated signature-enabled API probe returned `400` without reflecting the supplied wallet and logged `wallet [REDACTED]`. The `ad1ad6c` source change has not yet been rebuilt into a runtime image. | Static sink/metrics/browser collector audit remains incomplete; the ignored synthetic chain E2E test retains public synthetic output only. | `4af04c7`, `67240d8`, `bf58b86`, `254fd50`, `ad1ad6c` | PARTIALLY FIXED |
