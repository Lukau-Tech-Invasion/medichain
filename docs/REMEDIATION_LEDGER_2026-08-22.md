...
| UI-001 | P3 | The patient wearable page force-casts typed API envelopes to UI arrays and calls the readings route with a patient ID although the API path parameter is a device ID. | `client/patient-app/src/pages/WearablesPage.tsx`, `client/shared/src/api/endpoints.ts`, shared wearable types, API wearable handlers | The patient suite passed 26 files / 82 tests, but emitted React's missing/duplicate-key warning from `WearablesPage`. | Source comparison confirms `getWearableDevices()` returns `{ success, devices, count }`, `getWearableReadings()` returns `{ success, readings, count }`, and `/api/wearables/readings/{device_id}` requires a device ID. The page force-casts both responses to arrays and passes `patient.healthId` to the latter endpoint. | No live wearable integration, browser flow, or authenticated API read-back. The UI mapping/aggregation contract must be designed and tested before implementation. | pending | STILL PRESENT |
