...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | Transactional access-request creation **and approval** now span `api/src/handlers/access_control.rs`, `patient_access`, and both repository implementations; deny, revoke, and 11 other mapped high-risk flows remain. | Isolated PostgreSQL fault-injection tests reserve outbox event IDs for creation and approval. Creation passes only when its business row rolls back; approval passes only when the request remains `pending` and no grant exists (`1 passed`, 422 filtered). A separate approval success-path test confirms one approved request, its grant, and its outbox event commit (`1 passed`, 424 filtered). | `cargo check -p medichain-api --message-format short` passes. Source inspection still identifies fail-open `audit outbox write failed` branches in emergency access/grants, RBAC, device lifecycle, identity claims, mobile records, and the access-control deny/revoke transitions. | Access-request creation and approval have direct database transaction proof. No restart/browser proof and no atomic coupling proof for deny, revoke, or remaining high-risk flows. This remains a release blocker. | pending | PARTIALLY FIXED |
