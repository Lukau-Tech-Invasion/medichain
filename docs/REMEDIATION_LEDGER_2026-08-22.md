...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | All patient access request/grant transitions now span their repository and audit event. Guardian-relationship revocation now also does; emergency grant authority is PostgreSQL-backed but its audit writes remain separate. Ten mapped fail-open flows remain. | Isolated PostgreSQL duplicate-event tests prove every patient-access transition rolls back; approval/denial/revocation normal paths also commit with their event. Guardian revocation now has a duplicate-event rollback test (`1 passed`, 430 filtered). | `cargo check -p medichain-api --message-format short` passes. Source inspection still identifies fail-open audit branches in emergency access/grants, guardian create/update, device lifecycle, identity claims, and mobile records. | Guardian revocation has direct rollback proof but no fresh normal-path event test. Emergency authority has restart/binding/revocation database proof, but not atomic audit proof. This remains a release blocker. | pending | PARTIALLY FIXED |
