...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SEC-001 | P1 | Anonymous routes disclosed wallet-linked identity before ownership proof and had only a process-local request throttle. | `api/src/auth_challenges.rs`, `api/migrations/20260821000001_auth_challenges.sql`, auth handlers/routes/types, and shared/portal auth clients | Focused challenge and JWT-field tests pass. `a65f19f` adds a PostgreSQL advisory-lock-protected per-wallet rolling budget of five challenges per minute; its six-way concurrent test passed with exactly five issues and one typed rate-limit result. A direct PostgreSQL replay test proves a challenge consumes once and the identical second consume is denied. | Current image manifest `sha256:c26180a1…` is healthy. Through Nginx, five valid-format challenge calls returned `200`; the sixth and seventh returned `429` with `AUTH_CHALLENGE_RATE_LIMITED`. Earlier runtime calls for two wallet addresses had matching non-identifying response shapes. | Browser wallet signing, valid signed proof, expiry, and broader database race verification not yet run. | `8a7a5e3`, `a65f19f`, pending replay-test commit | PARTIALLY FIXED |
