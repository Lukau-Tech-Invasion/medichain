...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | All patient access request/grant transitions—creation, approval, denial, and revocation—now span `api/src/handlers/access_control.rs`, `patient_access`, and both repository implementations; 11 other mapped high-risk flows remain. | Isolated PostgreSQL fault-injection tests reserve outbox event IDs for every patient-access transition. Each passes only when its business transition rolls back; approval additionally has a separate success-path test confirming the request, grant, and event commit. | `cargo check -p medichain-api --message-format short` passes. Source inspection still identifies fail-open `audit outbox write failed` branches in emergency access/grants, RBAC, device lifecycle, identity claims, and mobile records. | All patient-access transitions have direct database rollback proof. Only approval has fresh success-path proof; creation/denial/revocation success, restart/browser, and the remaining high-risk flows remain unverified. This remains a release blocker. | pending | PARTIALLY FIXED |
...
* Access-denial atomic-outbox probe: `cargo check -p medichain-api
  ...
  ID leaves the request `pending`. A corresponding normal PostgreSQL success
  test has not yet run, so this establishes fail-closed rollback only.
* Access-revocation atomic-outbox probe: `cargo check -p medichain-api
  ...
  ID leaves the grant `active`. A corresponding normal PostgreSQL success test
  has not yet run, so this establishes fail-closed rollback only.
