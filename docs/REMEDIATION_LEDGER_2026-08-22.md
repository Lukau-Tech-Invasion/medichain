...
| AUD-001 | P1 | High-risk business mutations and durable audit-outbox inserts are often separate operations; the handler logs outbox persistence failure and still returns a successful mutation. | All patient access request/grant transitions—creation, approval, denial, and revocation—now span `api/src/handlers/access_control.rs`, `patient_access`, and both repository implementations. Emergency grant authority state now uses PostgreSQL when that backend is selected, but its audit writes remain separate; 11 mapped fail-open flows remain. | Isolated PostgreSQL fault-injection tests reserve outbox event IDs for every patient-access transition. Each passes only when its business transition rolls back. Fresh normal-path tests confirm approval, denial, and revocation each commit business state and one outbox event (`3 passed`, 426 filtered). | `cargo check -p medichain-api --message-format short` passes. Source inspection still identifies fail-open `audit outbox write failed` branches in emergency access/grants, RBAC, device lifecycle, identity claims, and mobile records. | All patient-access transitions have direct database rollback proof; approval, denial, and revocation also have normal commit proof. Emergency authority state has restart/binding/revocation database proof, but not atomic audit proof. This remains a release blocker. | pending | PARTIALLY FIXED |
