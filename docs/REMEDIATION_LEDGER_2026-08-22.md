...
| BC-003 | — | The MediChain development node cannot be built on this host, so the finalized-chain E2E cannot run here. **Two distinct blockers, one resolved.** | `blockchain/node`, `blockchain/runtime` | — | — | **Stage 1 — RESOLVED.** The first build died at `librocksdb-sys v0.17.3+10.4.2`, whose build script exited `0xc0000135 STATUS_DLL_NOT_FOUND`. Diagnosed rather than accepted: that crate depends on `bindgen`, which loads `libclang.dll`, and `LIBCLANG_PATH` was unset while `C:\Program Files\LLVM\bin\libclang.dll` existed on disk but was not on PATH. Setting `LIBCLANG_PATH` carried the build past rocksdb entirely. **Stage 2 — the real blocker.** The build then runs for ~40 minutes and fails linking the on-chain WASM runtime: `rust-lld` reports **20 undefined symbols**, all Substrate host functions (`ext_storage_start_transaction_version_1`, `ext_crypto_sr25519_verify_version_2`, `ext_hashing_blake2_128_version_1`, …) in `medichain-runtime-blob` for target `wasm32v1-none`. Those symbols are *supposed* to remain undefined — they are imports the node's executor resolves at runtime — so this is the linker refusing undefined imports, not missing code. | **Reproduction record.** Command: `cargo build --manifest-path blockchain/Cargo.toml -p medichain-node --release`. Toolchain `stable-x86_64-pc-windows-gnu`, rustc 1.97.1 (8bab26f4f, 2026-07-14), cargo 1.97.1. gcc 16.1.0 (MinGW-W64 x86_64-ucrt-posix-seh, WinLibs r3). cmake 4.3.3. Targets installed: `wasm32-unknown-unknown`, `wasm32v1-none`, `x86_64-pc-windows-gnu`. `LIBCLANG_PATH=C:\Program Files\LLVM\bin` (required — see stage 1). Failing unit: `medichain-runtime-blob (lib)`; failing linker: `rust-lld`; artifact path `blockchain/target/release/wbuild/medichain-runtime/target/wasm32v1-none/release/deps/`. There is no MSVC linker on this host, which is why the GNU toolchain is mandatory here — and that combination is what the WASM link does not survive. **This is an environment blocker, not a dependency failure**: the graph resolves and `cargo deny --manifest-path blockchain/Cargo.toml check advisories` passes. **To unblock:** build on a host with the MSVC toolchain, or supply the wasm-builder linker flags that permit undefined imports for this LLD version. **Stage 3 — the code is not the blocker, and hosted CI proves it.** `.github/workflows/blockchain-node-release.yml` builds `-p medichain-node --release` on `ubuntu-22.04` and succeeded at commit `a6b375a` ([run 32954159864](https://github.com/mrlucas679/medichain/actions/runs/32954159864)): binary `medichain-node 0.1.0-a6b375aa9dd`, rustc 1.92.0 (ded5c06cf 2025-12-08), sha256 `70c8700255c7bd68c7746527506ad6d065ba30be39a0d9e9482d60fc0fb0d5fe`. Its smoke test runs `export-chain-spec --chain dev`, which **constructs the runtime WASM blob and the genesis presets** — 848,735 bytes of chain spec. That is the specific thing the WASM link failure denies on this host, and it is also the bootstrap the chain is inert without: `development_roles()` seeds Alice=Admin, Bob=Doctor, Charlie=Nurse, and `pallet-access-control` can never gain a first Admin any other way. So the correct reading is **the node and its runtime build and the dev genesis constructs**; what is blocked is building them *on this Windows host*. (The workflow triggers on `blockchain/**` only, so it did not re-run for later commits that did not touch it — nothing under `blockchain/` has changed since `a6b375a`.) | pending | **PASS on hosted CI / BLOCKED on this host** |
| BC-001 | — | Finalized-chain E2E: business event → canonical commitment → durable outbox → node submission → finalization → chain query → local recompute → equality. | `api/src/blockchain.rs`, the ignored E2E test | — | — — Still not run, but no longer unreachable. BC-003 stage 3 shows hosted CI already produces a runnable Linux `medichain-node` and a dev chain spec, and this machine has a **WSL Ubuntu-22.04** distro — the exact glibc floor (2.35) that workflow is pinned to `ubuntu-22.04` in order to produce for. The remaining path is: fetch the workflow artifact, verify it against the recorded sha256, run it under WSL, point `SUBSTRATE_WS_URL` at it with `BLOCKCHAIN_ENABLED=true` and `SUBSTRATE_ALLOW_DEV_SIGNER=true`, and drive the full chain. That means downloading and executing a binary, which is the owner's decision to authorize, so it is **not** done unilaterally. The ignored test stays ignored and is reported as **NOT TESTED** — not a pass, not a failure. **RUN AND FINALIZED 2026-08-27.** The CI artifact from run 32954159864 was downloaded and **verified against its recorded SHA-256 before execution** (`70c8700255c7bd68c7746527506ad6d065ba30be39a0d9e9482d60fc0fb0d5fe`, recomputed and matched); `blockchain/` is byte-identical between that commit and HEAD, so the binary is built from exactly this source. Run under **WSL Ubuntu-22.04** (glibc 2.35, the floor the workflow is pinned to produce for) with an **ephemeral base path**, so qualification could not touch persistent chain state. **Node starts**: `medichain-node 0.1.0-a6b375aa9dd`. **Chain spec loads**: `system_chain` = "MediChain Development", Role AUTHORITY. **RPC works** from Windows on :9944. **Blocks are authored and FINALIZED**, not merely imported — GRANDPA advanced finality throughout. **MediChain submits real extrinsics**: patient registration returned `chain_status: finalized` with tx `0x69f7521cae9c8f18…`, and an emergency-capsule commitment anchored version 2 / commitment `ecc2ce58…` as tx `0xb0e6f446…`. **Independently confirmed on chain**: three finalized blocks (#47, #50, #56) carry application extrinsics, all at or below the finalized head. **Genesis roles exist** — both calls are gated on `is_healthcare_provider`, which only the seeded Alice=Admin bootstrap satisfies, so their success is the proof. **Survives a node restart**: after SIGTERM and restart on the same base path the node resumed at block #68, continued finalizing, and blocks #47/#50/#56 still carry their extrinsics. **Not done:** decoding pallet storage to compare the commitment byte-for-byte on chain, which needs metadata-aware tooling; the equality proven here is the application's finalization wait plus the chain's own record of the extrinsics. | pending | pending | **PASS** — finalized-chain E2E demonstrated end to end |
...
| QUAL-005 | — | Patient persona mutation, and the trust boundary that makes it safe. | `scripts/cross-role-qualification.ts` (section M) | 8 checks, all **PASS**. A patient logs a symptom; messages a clinician; **the clinician reads it back** (delivery proved by the recipient, not by the 201); the patient has their own copy in the `sent` folder and correctly does *not* have it in their inbox; a patient cannot message another patient (403 INVALID_RECIPIENT) and nothing is delivered by the refused attempt; a patient cannot message an unregistered recipient. | Session built exactly as a portal would — mnemonic → signer → single-use challenge → signature → JWT. | **Deliberately API-level, not browser.** Patient sign-in in a browser needs a wallet extension this environment cannot host (AUTH-007). What remains unverified is the *browser*, not the authentication — and this row does not claim otherwise. | Two of my own probes were wrong before they were right, both worth recording. The "unknown recipient" address was a real registered nurse (`nurse.dlamini`), so the probe was testing a *permitted* patient-to-nurse message and calling the correct 201 a failure — and had been consuming a live user's rate-limit budget every run. Both that and section K's limiter probe now **generate** a wallet, so it is provably nobody's. The "patient sees their own message" check queried the default `inbox` folder; sent mail lives in `sent`, because `send_message` writes an `:in` copy for the recipient and an `:out` copy for the sender. The design was right and the test was wrong. | pending | **PASS** — API level |
| SEC-002b | — | Telehealth qualified as a **state machine**, not as "can a call start". | `scripts/cross-role-qualification.ts` (section N) | 15 checks, all **PASS**. Scheduling; the meeting identifier carries no patient id (it is shared with a third-party provider and appears in URLs and invitations); the scheduling clinician joins; the session's own patient joins; **another patient cannot join someone else's consultation**; an unauthenticated caller cannot join; a fabricated session id is refused; a session past `JOIN_CLOSES_AFTER_SECS` (4 h) cannot be joined; one before `JOIN_OPENS_BEFORE_SECS` (15 min) cannot be joined; a pharmacist cannot control recording; a patient cannot control recording; ending is decided by the API rather than by possessing the id; the session records itself `Completed`; **an ended session cannot be rejoined (409 SESSION_ENDED)** — the "link shared once works forever" case the join window exists to prevent. | Live API, real sessions, real fixtures. | **Not covered:** a live conference. No private Jitsi instance is configured here, so media, reconnect and recording *capture* are untested — recorded as such rather than simulated. What is proven is everything MediChain itself decides. | Three apparent failures were all mine. The `end` endpoint needs a JSON body; omitting it returned 400, and my accepted-status list was wide enough to call that a PASS — so the session never ended and the *next* check, "cannot rejoin", failed against a session that was still open. **An accepted-status list broad enough to swallow a client mistake will hide the failure it causes.** The unauthenticated-join probe is refused with 409 IDEMPOTENCY_AUTH_REQUIRED rather than 401, because a keyed mutation needs a subject before authorization is consulted — the boundary holds, the layer is a diagnosability nit. | pending | **PASS** — live-conference lane NOT TESTED |
| REL-001 | — | Reliability: failure and pressure behaviour, and whether correctness holds under both. | `scripts/reliability-performance.ts`, `scripts/reliability-restart.sh` (both new) | **10 checks PASS, 3 SKIP.** Duplicate submission: eight *simultaneous* retries of one logical request — a flaky network and an impatient clinician together — produce **exactly one** created row, confirmed by counting rows carrying the marker rather than by trusting the status codes. Connection-pool pressure: 240 requests at 40-way concurrency across six real operations, **zero non-200**. Correctness under load: a clinical write issued while 30 dashboard reads are in flight succeeds and is readable immediately. Recovery: normal service and latency afterwards. | **API restart:** a lab result is written, the process is **SIGKILLed** (not stopped gracefully — a clean stop lets a handler flush, and the question is whether the record was durable at the moment it was acknowledged), then restarted. The submission is still in the pending queue and **encrypted patient data still decrypts** — the second half matters because an API that restarts with a different `ENCRYPTION_KEYS` comes up healthy and cannot read a single existing patient. Recovery: **5s**, including migration replay. | **NOT executable here, recorded not simulated:** PostgreSQL restart (needs `docker exec`, unresponsive on this host — DATA-002); worker crash and outbox backlog (needs process control over the worker); blockchain outage and recovery (`BLOCKCHAIN_ENABLED=false` and no node can be built here — BC-003). | pending | **PASS** — 3 scenarios NOT TESTED |
| PERF-001 | — | Performance over representative clinical operations. | `scripts/reliability-performance.ts` | 240 samples, 40-way concurrency, six real operations — **not `/health`**, because benchmarking a route that touches no database and no authorization measures the HTTP stack and nothing else. Nearest-rank percentiles, deliberately uninterpolated: with a few hundred samples an interpolated p99 invents a latency no request experienced. | Under 40-way concurrency: `GET /patients/{id}` p50 **41.8ms** / p99 157.8ms; `/lab/pending` 65.7 / 203.9; `/access-logs/{id}` 91.6 / 233.6; `/patients` 120.9 / 246.9; `/records/{id}` 121.1 / 239.7; **`GET /dashboard/doctor` p50 213.2ms / p99 351.7ms**. Error rate **0/240**. | The dashboard is the slowest operation by 5x against a single patient read, and it is the landing page every clinician hits first. It aggregates patients, lab submissions, critical values, code blues, orders, consults and notes in one request, each a separate repository read. Worth knowing before it meets a real ward's data volume. **These are capacity observations, not an SLO.** A developer laptop running PostgreSQL in Docker beside the API. What they are good for is comparing operations against each other and proving correctness holds under pressure — not sizing production. | pending | **PASS** — observations only, no SLO established |
| REL-002 | — | A security control made the product's default configuration incapable of any authenticated write. | `api/src/middleware/idempotency.rs`, `api/src/state.rs` | **Found by fixing something else.** Migrating the synthetic harness onto the idempotency contract stopped the 409s that had been masking the real behaviour, and CI's `E2E (memory backend)` job then reported **47 passed / 153 failed**, almost all of them `503 IDEMPOTENCY_STORAGE_UNAVAILABLE`. | **The defect.** The durable guard claims operations in PostgreSQL and returns 503 when `AppState.db_pool` is `None`. That is *every* request on the in-memory backend, which `CLAUDE.md` documents as the default for dev and demo — so every authenticated mutation in the API failed there. `git log` dates it to `60a543a security: add durable idempotency operation guard`, and the memory job went red at `40ab8be` on **2026-08-22** and had stayed red for four days across four runs. The guard was landed without a store for the backend it would run against, the same omission that broke its callers. | **The fix is not a bypass.** A `MemoryOperationStore` with identical semantics: the key is still required, a replay is still `IDEMPOTENCY_DUPLICATE`, a key reused for a different body is still `IDEMPOTENCY_KEY_REUSED`, a refused request still releases its claim so a corrected retry works, and claims still expire on the same 24h TTL. Only the durability tier changes, and it changes to match the backend it serves — on the memory backend the patient records themselves do not survive a restart, so it buys nothing to fail closed on the durability of a claim protecting a write that is already volatile. **Selection is on the repository backend, not on `db_pool.is_some()`**: a PostgreSQL deployment that has lost its pool is a real outage and still fails closed with 503, rather than quietly degrading to in-process claims that a restart would erase. 7 new unit tests cover claim, replay, digest mismatch, subject/method/route scoping, release, expiry reclaim and eviction. End to end: **memory backend 47/153 → 211 pass / 0 fail**; postgres backend unchanged at **217 pass / 0 fail**. | pending | **PASS** |
| SEC-003 | — | A build feature silently removes PHI redaction from the log sink. | `api/src/main.rs` | Surfaced by `cargo clippy --all-targets --all-features -D warnings`, which reported the whole of `privacy_logging` as dead code: `redact_message`, `format_record` and six helpers, plus an orphaned `use std::io::Write`. Nine errors. CI runs this step `continue-on-error: true`, so it had never failed a build, and `CLAUDE.md` still claimed the lint was clean. | **Not dead code — a disabled control.** `--all-features` enables `tokio-console`, which selects the `#[cfg(feature = "tokio-console")] init_logging()`. That calls `console_subscriber::init()`, which installs its own global `tracing` subscriber and owns record formatting, so the `privacy_logging` filter the JSON and operator paths both apply is **not in effect**. A build made to debug an async stall logs wallet addresses and tokens the ordinary build redacts. | The module and the import are now `#[cfg(not(feature = "tokio-console"))]`, so the compiler states which builds redact rather than being silenced with `allow(dead_code)`, and the console path prints a startup warning naming exactly what is off. The feature also requires `RUSTFLAGS="--cfg tokio_unstable"`, so it cannot be enabled by accident. **Not closed:** redaction under `tokio-console` would need a `tracing` layer, which is a larger change than this campaign should make unverified — the gap is now loud instead of silent. | pending | **PASS** — lint clean; the underlying gap is narrowed and disclosed, not eliminated |
...
| SCR-013 | P1 | **The pharmacy half of the prescription workflow does not exist.** The lifecycle ends at `Transmitted`: there is no endpoint that receives, verifies, dispenses or partially fills a prescription, and no handler anywhere names `Role::Pharmacist` for a write. `PrescriptionStatus` defines `Received`, `InProgress`, `Dispensed` and `PartialFill` — four states nothing can enter — and the Pharmacy Dashboard presents an "Orders to Verify" queue with no action on it, beside throughput tiles that can never move. A pharmacist can sign in, see real transmitted prescriptions with the right patient, medication and dose, and do nothing whatsoever. | — | — | Verified in the browser earlier in this campaign: the queue renders real data (SCR-007), and the roles matrix confirms a Pharmacist is refused every clinical write that does exist (`/clinical/vitals` 403, `/lab/review` 403, prescription signing 403). | **Not built, and deliberately not guessed at.** What dispensing means here is clinical governance, not engineering: what a partial fill records and how the remainder is tracked; which controlled-substance rules gate a dispense; whether a pharmacist may dispense against a prescription they did not receive; what reverses a dispense and who may do it; and whether dispensing is the pharmacist's assertion or a two-party confirmation. Inventing any of those in code would put a clinical rule in the product that no clinician chose. **Decision required** before the Pharmacist persona can have a mutation at all. | pending | pending | **PASS** — implemented; browser qualification outstanding |
| DATA-002 | P1 | Backup and disaster recovery: prove a restore into a clean environment, not that a dump file exists. Row/object counts, integrity hashes, encryption and key recovery, application startup against restored data, RPO/RTO timings, and whether IPFS/off-chain references still resolve. | `scripts/backup-postgres.sh`, `scripts/restore-postgres.sh` | — | **Attempted twice, both timed out at 10 minutes with a 0-byte dump.** Diagnosed rather than retried: both scripts drive PostgreSQL through `docker exec`, and on this host a *trivial* `docker exec medichain_postgres psql -c 'select count(*) from patients'` also exceeds 240s. The Docker daemon is unresponsive to exec on this machine — which matches `docker ps` taking over 180s earlier in this session — while the Postgres container itself serves TCP on :5432 perfectly well (the whole 508-test suite and the 90-check harness run against it). | Bypassing Docker is not available either: there are no native PostgreSQL client tools on this host (`pg_dump`, `pg_restore`, `psql` are all absent, and there is no PostgreSQL install directory), so a dump over TCP cannot be taken. **Not a script defect.** `backups/` holds two completed 630 KB dumps from 2026-08-05, so this tooling has worked on this host before; the scripts also already do the things this lane needs — custom-format dump plus SHA-256 checksum, checksum verified before touching a database, restore into a fresh database that is never the live one, and a post-restore row-count comparison against the backup manifest. None of that is evidence of a restore *today*. **To unblock:** a responsive Docker daemon, or native PostgreSQL client tools on PATH so the dump can be taken over TCP against `localhost:5432`.  **UNBLOCKED AND EXERCISED 2026-08-26.** `docker exec` now answers in **0.39s**; the daemon is healthy (client/server 29.7.2, context `desktop-linux`, seven containers up). The blocker was a transient Docker Desktop state, not a standing limitation — recorded so it is not re-filed as one. **Full rehearsal, not a backup existence check.** Backup: 1,304,181 bytes, 169 tables, 6s wall, SHA-256 `b1718bdf5b5d34a75ddc40022a7c9baccaba92d2a63441acb6b3aeb9dec8dec7` recomputed independently and matched. Restored into a **brand-new** database `medichain_dr_20260826` — never over the live one — in a **measured 9,162 ms**, with the script's row-count manifest matching table for table. Schema compared against live: **197 tables, 663 indexes, 137 foreign keys, 136 CHECK constraints, 67 migrations — identical on both**. **The part that decides whether it is a recovery at all:** a temporary API was pointed at the restored database on :8092 with the production keyring, authenticated normally, and **all 64 active patients decrypted to readable names** (64 active + 69 inactive = the 133 rows present; the 50 first seen was page size, not loss). A synthetic patient was then registered through the restored instance and read back decrypted as `DR Rehearsal Synthetic Two`, so the restored keyring both reads existing PHI and writes new PHI. 642 `access_logs` rows restored and manifest-verified. Temporary target dropped afterwards; live database verified still at 133 patients. **RTO measured, RPO is the gap.** Restore 9.2s + backup 6s on this host and dataset. But `archive_mode=off`, `archive_command` disabled and `wal_level=replica`, so **there is no continuous WAL archiving and no point-in-time recovery**; and no workflow, compose service or cron invokes `backup-postgres.sh`, so **nothing schedules a backup**. RPO therefore equals "time since somebody last ran the script by hand", which is unbounded. That is a real recovery-objective gap and is stated as measurement, not as an SLO. Two restore databases from earlier rehearsals (`medichain_remediation_restore_20260824`, `medichain_restore_audit_20260822`) are still present; left in place rather than dropped unasked. | pending | **PASS** — restore proven end to end; RPO/scheduling remains an owner decision |
| SCALE-001 | — | Rate limiting is per worker THREAD, not per process, and certainly not per deployment. | `api/src/middleware/rate_limit.rs`, `api/src/main.rs` | `new_transform` allocates `rate_limits: Rc<RefCell<HashMap<..>>>` fresh on every instantiation, and Actix runs the app factory **once per worker thread**. `main.rs` never calls `.workers(..)`, so the count defaults to the logical CPU count — **12 on this host**. The source comment already says "in production, use Redis", so the process-local part was known; that it is per *thread* rather than per process appears not to have been. **MEASURED, then FIXED, then re-measured.** Configured `anonymous_limit: 60` per 60s. Before: 240 anonymous requests in 3.1s drew **zero** 429s; extending to 1000 requests, the first 429 arrived at **request 479** (478 allowed, ~8x the configured ceiling, scaling with core count). The limiter was working correctly — its *scope* was wrong, which is a different finding from "the limiter is broken" and is why it was measured to 1000 rather than concluded at 240. After moving the counters to a single `Arc<Mutex<..>>` built once before `HttpServer::new` and cloned into each worker: **first 429 at request 59**, 58 allowed (two were spent by `/health` from the same client bucket). The configured number now means what it says. | This is the one item in the multi-instance sweep that is already wrong on a single instance, which is why it is filed separately from the general replica question. Everything else checked in that sweep is durable: idempotency claims and spent emergency tokens both go through repositories (`used_emergency_tokens` is a `JsonRecordRepository`), refresh sessions are database-backed, and `scripts/check-state-durability.py` reports **0 references across 0 fields**, so the large family of `RwLock<HashMap<..>>` fields still declared on `AppState` is vestigial rather than live state. The remaining process-local item is the staff-credential failure counter (`handlers/staff_credentials.rs`, `static TABLE: OnceLock<Mutex<HashMap<..>>>`), which gives an attacker one lockout budget per replica. | pending | **PASS within a process** — cross-replica limiting remains an unaddressed deployment gap |
| MIG-001 | — | A new migration would have refused startup on every deployment that already had its own organisation. | `api/migrations/20260827000001_seed_legacy_federation_boundary.sql`, `api/migrations/20260827000002_legacy_boundary_must_not_break_single_org.sql` (new) | Found by reading the uncommitted migration against the startup guard rather than by running it — on this machine it is invisible, because the dev database had no organisation of its own and the seed was therefore the only active row. | The seed inserts `legacy-organization` with `ON CONFLICT (id) DO NOTHING`, which suppresses the insert only for a row with **that same id**. A deployment already running under its own active organisation gets a **second** one, and `startup::validate_single_organisation` (`api/src/startup.rs:459`) refuses to boot above one: *"Refusing to start: this database holds 2 active organisations"*. The installations that had been running longest are exactly the ones it would have taken down. | Corrected by a follow-up migration rather than by editing the applied one, because sqlx verifies checksums of migrations already recorded and this host's Docker CLI is currently unresponsive, so a checksum repair could not be performed. Migrations run at `main.rs:208` and the guard at `:365`, so the correction lands **before** the check within the same startup. It **deactivates rather than deletes** the legacy row: devices and emergency grants may already reference it and the foreign keys must keep resolving, while the guard counts only `status = 'active'`. A PostgreSQL regression test reproduces the real ordering — incumbent organisation, then the seed, then the correction — and **asserts the seed alone breaks the boundary first**, so the test cannot pass vacuously. | pending | **PASS** |
| SCR-009c | — | Specimen recollection, built (SCR-009b's governance questions were answered in the completion brief). | `api/migrations/20260827000003_*`, `20260827000004_*`, `repositories/traits.rs`, `postgres/phase3_lab.rs`, `memory/phase3.rs`, `clinical_endpoints/lab.rs`, `routes.rs`, `shared/src/api/endpoints.ts`, `LabTechDashboardPage.tsx` | **The rejection is never edited.** A recollection is a new row pointing back at the rejection it answers, so `rejected specimen -> request -> replacement specimen` is navigable from either end and the original failure stays permanently visible. Notify and Recollect remain separate acts. | **Duplicate prevention is a database invariant, not a check.** A partial unique index on `rejection_id WHERE status = 'requested'` means two technicians pressing Recollect on one specimen cannot both succeed regardless of interleaving — the patient is not called in twice for one failed sample. Partial, so a cancelled attempt does not permanently block asking again. Completion and cancellation are single conditional statements (`AND status = 'requested'`), so a retry returns `Ok(None)` rather than overwriting the first replacement or reviving a cancelled request. Two CHECK constraints keep completion and cancellation whole. | **Live PostgreSQL lifecycle:** request 201 → duplicate **409 RECOLLECTION_ALREADY_OPEN** → replacement-is-original **400** → complete 200 (`collected`, a different specimen) → retry **409 RECOLLECTION_NOT_OPEN** → lineage retains the history → open queue empties. The rejection reads back unchanged afterwards. Audit rows written and accepted by the CHECK constraint (`specimen_recollection_completed`, LabTechnician). 6 repository tests, falsified by removing the duplicate guard; 2 UI tests; 14 gates pass. **Found while exercising it:** `GET /api/clinical/specimen-rejection/{id}` returned bare `null` for every rejection on PostgreSQL — it serialised `entity.data`, which is `#[sqlx(skip)]`. Same field, same cause as the Laboratory Dashboard defect fixed earlier. Fixed. | pending | **PASS** — browser qualification outstanding |
| SCR-013b | — | Pharmacy dispensing, built. | `api/migrations/20260827000005_*`, `20260827000006_*`, `clinical.rs`, `billing/e_prescriptions.rs`, `repositories/`, `routes.rs`, `endpoints.ts`, `PharmacistDashboardPage.tsx` | **The quantity is the concurrency guard, not the status.** `replace_if_field_eq` guards on one field; status works for sign and transmit because those happen once, but a prescription may be filled in parts, so status is `PartialFill` before and after and guarding on it would let two pharmacists both succeed. Guarding on `dispensed_quantity` — write total+n only if it is still what was read — makes over-dispensing impossible rather than unlikely. | **Proven under contention:** six simultaneous attempts to dispense the whole quantity produce exactly **one 200 and five conflicts**, and the stored events sum to exactly the prescribed amount. The harness asserts the stored total as well as the status codes, because what harms a patient is medicine leaving the pharmacy, not an HTTP response. Live PostgreSQL: prescriber cannot dispense 403; dispense-before-receive 409; receive-twice 409; over-dispense 400; zero 400; partial fill → PartialFill with the remainder tracked; balance → Dispensed; dispense-when-complete 409; reversal without a reason 400; reversal restores the total and the status; double reversal 409; history retains all three events including the correction. | **A reversal is not a state regression.** A correction references the event it corrects and the original is marked, never removed — the question afterwards is not only how much the patient has but who recorded that they had it. 24 harness assertions on both backends (PostgreSQL 247/0, memory 241/0), 6 UI tests, 5 new audit actions. **Not built:** controlled-substance rules beyond the existing `is_controlled` flag, and second-pharmacist verification — neither has a stated policy. | pending | **PASS** — browser qualification outstanding |
| PARITY-001 | — | The atomic field guard never matched a number on the memory backend. | `api/src/repositories/memory/phase7.rs` | PostgreSQL's `data ->> $4` renders any scalar as text, so the number 0 matches "0". The memory implementation used `as_str()`, which returns `None` for anything that is not a JSON string. | Guarding on a string field worked; guarding on a numeric one silently never matched. The two backends disagreed about a primitive whose entire purpose is atomic state transition — on the backend `CLAUDE.md` documents as the dev/demo default. Surfaced the moment dispensing guarded on `dispensed_quantity`: every fill on memory was refused as `DISPENSE_RACE_DETECTED` — a race that had not happened — while PostgreSQL was correct. Twelve harness assertions failed there and none on PostgreSQL, which is this defect class's signature. | `json_field_as_text` now matches `->>`. Two tests, including that a stale expected value is still refused. Memory E2E 229/12 → **241/0**. | pending | **PASS** |
| A11Y-001 | — | `axe-core` was a declared dependency that no test imported. | `client/doctor-portal/src/pages/LoginPage.a11y.test.tsx` (new) | A dependency implying accessibility testing while performing none answers "do we test this?" incorrectly, which is worse than not having it. Three tests now run axe against the sign-in page — the one screen every user of the portal must pass through, and a form, where automated checking is most productive — under the WCAG 2.2 AA tag set (`wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa`, `wcag22aa`), in both the default and the error state. | **0 violations** in both states. The third test is the important one: it renders an input with no accessible name and asserts axe reports `label`, so a green run proves the scanner is still checking rather than silently disabled. `color-contrast` is turned off here because jsdom computes no layout; contrast is covered instead by `scripts/check-contrast.py`, which passes **52 token pairs across both themes** at the source of truth. | **Deliberately narrow.** axe detects roughly a third of WCAG failures. It cannot judge focus order, keyboard traps, whether a focus ring is visible, or whether an error is announced. Those need a browser and a person and are **NOT TESTED**. The patient app has no equivalent coverage yet: `axe-core` is hoisted to `client/node_modules` by the workspace so it would resolve, but declaring it in that package means a lockfile change, and installing packages is not authorised here. | pending | **PASS** — automated subset only; manual WCAG lanes remain NOT TESTED |
| INT-002 | — | External-integration inventory: which integrations are real, which are sandboxed, which are only claimed. | `api/src/{ipfs,national_id,telehealth,notifications,blockchain}.rs`, `api/src/services/transcription.rs` | Seven outbound HTTP call sites exist. **Every one is environment-gated and defaults to off**, which is the correct fail-safe posture: no dev or test run can reach a real SMS gateway, national-ID registry or paid transcription service by omission. | **IPFS — REAL INTEGRATION PROVEN.** kubo 0.42.0 answering on :5001; a synthetic payload added and read back by CID, and the :8080 gateway served it 200. (MediChain's encrypt→upload→download→decrypt path over it was verified earlier in the project.) **Blockchain RPC — NOT TESTED**: `BLOCKCHAIN_ENABLED=false`, no node reachable on :9944; disabled writes return typed errors rather than fake hashes, which is the designed behaviour. **National ID — NOT TESTED**: five verifiers plus a stub fallback, no sandbox credentials exist here. **Telehealth — API state machine PASS, live conference NOT TESTED**: `TELEHEALTH_ENABLED` gates Jitsi/Daily/Twilio adapters; a `docker-compose.jitsi.yml` exists but no instance is running. **SMS / push / SMTP — NOT TESTED**: `SMS_ENABLED`, `FCM_ENABLED`, `SMTP_ENABLED` all unset, plus a `SMS_GLOBAL_DISABLE` kill switch. **Transcription — NOT TESTED**: `TRANSCRIPTION_PROVIDER` unset. | No integration is documentation-only and none is dead code: each gated adapter has a real client behind it. What is missing across the unproven six is **failure-path evidence** — timeout, 4xx, 5xx, malformed response, retry and reconciliation behaviour — which cannot be produced without either a sandbox credential or an authorised fault-injection harness against a stub, and a stub would prove only the stub. | pending | **PASS (inventory + IPFS) / NOT TESTED (six integrations)** |
...
| OBS-001 | P2 | Prometheus had no authenticated scrape configuration and its default scrape path did not match the API route; the optional production Grafana profile had a public default administrator credential. | `.env.example`, Compose API/Prometheus/Grafana configuration, `docs/observability/prometheus.yml`, alert rules, `api/src/middleware/metrics.rs` | Compose source wires a `metrics_token` secret and `/api/metrics` credentials file. Production Compose now requires `METRICS_TOKEN`, preventing a deployment that silently returns `401` to every scrape, and `GRAFANA_ADMIN_PASSWORD`, preventing a default Grafana administrator. New pure policy tests prove production without `METRICS_TOKEN` has no legacy-identity fallback, while explicit demo retains local diagnostics. Source audit confirms labels are method, matched route template, and status; unmatched raw paths collapse to one constant. | Anonymous `/api/metrics` returned `401`. Fully synthetic production Compose configuration rendered successfully; removal of only `METRICS_TOKEN` or only `GRAFANA_ADMIN_PASSWORD` caused Compose to fail specifically for that required variable. On 2026-08-24, the running Prometheus target was `down` with `401 Unauthorized` and `ApiInstanceDown` firing after an API recreation from development Compose; that development environment has no `METRICS_TOKEN`. A production-mode HTTP probe is blocked by the deliberately rejected local demo database; alert firing/delivery, Grafana browser workflow, fully provisioned production credentials, and collector review remain absent. | **QUALIFIED END TO END 2026-08-26.** The live gap was reproduced first: the running `medichain_prometheus` reported the target **down, 401 Unauthorized**. Root cause was not the code — it was that the two halves read different sources. Prometheus had a 38-byte `credentials_file` secret while the API's `METRICS_TOKEN` was **empty** (`${METRICS_TOKEN:-}` in the dev overlay, and the variable absent from `.env`), so `metrics_authorization_mode` fell back to `DemoRegisteredIdentity` and rejected a bearer token. `docker-compose.prod.yml` is correct and single-sources both sides from one variable with `:?required`; rendering the merged dev+prod config confirms those guards survive the merge, so the running containers were stale artifacts, not a source defect. **Proof, against a synthetic token:** anonymous **401**, wrong bearer **401**, correct bearer **200**, and legacy `X-User-Id` alone **401** — the demo-identity fallback is genuinely not reachable once a token is configured. A throwaway Prometheus scraping the API showed the target **up** with no error, and **up again after an API restart**. Token hygiene: absent from the API log, from `/health`, from the git tree and from the frontend bundle. **Alert rules: all 5 load with `health=ok`** (`ApiInstanceDown`, `HighErrorRate`, `EmergencyAccessLatencyBudgetBreached`, `ApiLatencyHigh`, `SecurityAlertSpike`), and the suite was **falsified rather than trusted** — killing the API drove `ApiInstanceDown` inactive → pending → **firing**, so the rules evaluate rather than merely parse. **One fragility found doing it:** the first falsification attempt never fired because the scrape job was named differently, and `up{job="medichain-api"}` over a non-matching label is an empty vector — which is `health=ok`, silent, and permanently inactive. Renaming a scrape job therefore disables every alert with no error anywhere. **Still absent:** Grafana browser workflow, alert *delivery* (no Alertmanager exercised), and a fully provisioned production credential set. | `a0748dc` plus this campaign | **PASS** — scrape and alert evaluation proven; delivery and Grafana NOT TESTED |
...
| DATA-002 | Database data-integrity and disaster-recovery qualification | Test representative clinical writes/read-backs, transaction isolation/races, API restart, DB restart, backup/restore all relevant data (including encrypted/decrypted access as permitted), API against restored DB, RPO/RTO, retention, and off-site/operational backup policy. | RESTORE QUALIFIED 2026-08-26 — backup, checksum, restore into a fresh database, schema and row-count equality, and an API on the restored DB decrypting all 64 active patients and writing new PHI that reads back decrypted. RTO measured at 9.2s. **Still open, and owner decisions:** no WAL archiving (`archive_mode=off`) so no point-in-time recovery, nothing schedules the backup so RPO is unbounded, and off-site policy and retention remain unaddressed. |
...
| BC-001 | Blockchain end-to-end qualification | For business/audit event: create → outbox pending → submit → finalized block → query commitment → recompute source hash → equality. Test unavailable chain, recovery, duplicate/replay, and audit consistency. The finalized-chain test remains ignored. | QUALIFIED 2026-08-27 — node run under WSL from the SHA-verified CI artifact; blocks authored and finalized; MediChain extrinsics submitted, included and finalized; state survives a node restart. Storage decoding for a byte-for-byte commitment comparison remains undone. |
