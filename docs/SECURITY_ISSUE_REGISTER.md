...
| SEC-26 | PostgreSQL path (`PARTIAL — first ever end-to-end run`) | Volume recreated on owner instruction: **40/40 migrations now apply**, 182 tables (was 121), all HZ-023 stores present. First real run exposed HZ-026: `nfc_tags.tag_uid` was VARCHAR(32) holding a 64-char SHA3-256 hash, so **no patient could register on PostgreSQL at all** — fixed by migration `20260804000002`, moving the suite 89→**147/160**. Memory backend stays 160/160. **13 assertions still fail on PostgreSQL** (users.valid_wallet vs patient-id wallets, io_records.recorded_by uuid-vs-text, medication_records.data missing). Do not call PostgreSQL supported until green. |
