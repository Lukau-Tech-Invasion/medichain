...
| SEC-26 | PostgreSQL path never run end-to-end (`ROOT CAUSE FOUND — HZ-025`) | Docker is now up, and the path still cannot run: **8 of 39 migrations are applied**. One already-applied migration was edited, and sqlx halts the ENTIRE chain on a checksum change; demo mode downgraded that to a single warning line, so the API reported healthy on a January schema (121 tables vs a documented 179). The three HZ-023 stores do not exist in PostgreSQL. Startup now reports applied-vs-available counts so it cannot be scrolled past. **Schema repair needs an owner decision**: recreate the volume (destructive) or repair the checksum drift. |
