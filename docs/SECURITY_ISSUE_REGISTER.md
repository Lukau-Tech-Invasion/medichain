...
| ID | Issue | Notes |
|---|---|---|
| SEC-11 | **Presence-only handlers: 144 → 49** (`PARTIAL`) | 95 fixed via three shared gates. `require_registry_reader` (15 platform registries — resolves, role-checks, **audits** the bulk read); `require_clinical_staff` (clinical documentation, orders, results, dashboards, surgical, emergency); `require_registered_caller` (patient-facing: sync, localization — resolves the caller but does **not** require a clinical role, which would lock patients out of their own features). Verified live: forged→401 everywhere, clinician→200, patient→403 on clinical endpoints but →200 on patient-facing ones. Remaining 49 are mostly per-resource handlers that need an ownership check, not just a role. Ratchet at 49. |
