...
mod tests {
    ...
    use crate::repositories::{
        PatientEntity, PatientRepository, Pagination,
        AllergyEntity, AllergyRepository,
        MedicalRecordEntity, MedicalRecordRepository,
    };
    use crate::repositories::postgres::{
        PgPatientRepository, PgAllergyRepository, PgMedicalRecordRepository
    };
    use chrono::Utc;
    use sqlx::PgPool;
    use std::env;
    ...
    async fn get_test_pool() -> PgPool {
        ...
        let database_url = env::var("DATABASE_URL")
            .unwrap_or_else(|_| "postgres://medichain:medichain_dev_2024@localhost:5432/medichain".to_string());
    ...
    async fn test_pg_patient_repository() {
        ...
        // Test Create
        let created = repo.create(patient).await.expect("Failed to create patient");
        assert_eq!(created.id, patient_id);
        
        // Test Get by ID
        let fetched = repo.get_by_id(&patient_id).await.expect("Failed to get patient by ID");
        assert_eq!(fetched.health_id, format!("HID-{}", patient_id));
        
        // Test Get by Wallet
        let fetched_wallet = repo.get_by_wallet(&format!("0x{}", patient_id)).await.expect("Failed to get by wallet");
        assert_eq!(fetched_wallet.id, patient_id);

        // Test Update
        let mut updated_patient = fetched.clone();
        updated_patient.blood_type = Some("A-".to_string());
        let updated = repo.update(updated_patient).await.expect("Failed to update patient");
        assert_eq!(updated.blood_type, Some("A-".to_string()));
        
        // Test List
        let list = repo.list(Pagination::new(0, 10)).await.expect("Failed to list patients");
        assert!(list.total >= 1);
        assert!(list.items.iter().any(|p| p.id == patient_id));

        // Test Search
        let search_results = repo.search(&patient_id, Pagination::new(0, 10)).await.expect("Failed to search");
        assert_eq!(search_results.total, 1);
        assert_eq!(search_results.items[0].id, patient_id);

        // Test Delete (Soft Delete)
        repo.delete(&patient_id).await.expect("Failed to delete patient");
    ...
    async fn test_pg_allergy_repository() {
        ...
        let patient = create_test_patient(&patient_id);
        patient_repo.create(patient).await.expect("Failed to create patient");
        ...
        // Test Create
        let created = allergy_repo.create(allergy.clone()).await.expect("Failed to create allergy");
        assert_eq!(created.allergen, "Peanuts");

        // Test Get by Patient
        let allergies = allergy_repo.get_by_patient(&patient_id).await.expect("Failed to get allergies");
        assert_eq!(allergies.len(), 1);
        assert_eq!(allergies[0].allergen, "Peanuts");

        // Test Has Allergen
        let has = allergy_repo.has_allergen(&patient_id, "Peanuts").await.expect("Failed has_allergen");
        assert!(has);

        // Test Update
        let mut updated_allergy = created.clone();
        updated_allergy.severity = "LifeThreatening".to_string();
        let updated = allergy_repo.update(updated_allergy).await.expect("Failed to update");
        assert_eq!(updated.severity, "LifeThreatening");

        // Test Delete
        allergy_repo.delete(&created.id).await.expect("Failed to delete");
        let active = allergy_repo.get_active_by_patient(&patient_id).await.expect("Failed to get active");
        assert_eq!(active.len(), 0);

        // Cleanup
        sqlx::query("DELETE FROM allergies WHERE patient_id = $1").bind(&patient_id).execute(&pool).await.ok();
        sqlx::query("DELETE FROM patients WHERE id = $1").bind(&patient_id).execute(&pool).await.ok();
    }
    ...
    async fn test_pg_medical_record_repository() {
        ...
        // Test Create
        let created = record_repo.create(record.clone()).await.expect("Failed to create record");
        assert_eq!(created.ipfs_content_hash, Some("QmTest123".to_string()));

        // Test Get by Patient
        let records = record_repo.get_by_patient(&patient_id, Pagination::new(0, 10)).await.expect("Failed to get records");
        assert_eq!(records.items.len(), 1);

        // Test Get by IPFS Hash
        let fetched = record_repo.get_by_ipfs_hash("QmTest123").await.expect("Failed to get by IPFS");
        assert_eq!(fetched.id, created.id);

        // Test Delete
        record_repo.delete(&created.id).await.expect("Failed to delete");
        let records_after = record_repo.get_by_patient(&patient_id, Pagination::new(0, 10)).await.expect("Failed to get records");
        assert!(records_after.items.iter().all(|r| !r.is_active));

        // Cleanup
        sqlx::query("DELETE FROM medical_records WHERE patient_id = $1").bind(&patient_id).execute(&pool).await.ok();
        sqlx::query("DELETE FROM patients WHERE id = $1").bind(&patient_id).execute(&pool).await.ok();
    }
}
