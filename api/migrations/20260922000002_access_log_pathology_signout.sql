-- =============================================================================
-- access_logs.action: signing a pathology report
-- =============================================================================
-- `update_pathology_report` writes an audit row and was never added to the
-- vocabulary, so on PostgreSQL the insert is rejected AFTER the report itself
-- has been written -- and audit writes here fail closed, which turns a missing
-- CHECK value into a refused clinical operation. In memory it succeeds, which
-- is why the endpoint's own tests are green.
--
