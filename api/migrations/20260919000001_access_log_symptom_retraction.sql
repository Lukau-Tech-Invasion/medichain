-- =============================================================================
-- access_logs.action: patient retraction of a symptom-diary entry
-- =============================================================================
-- `POST /api/symptoms/{patient_id}/{entry_id}/retract` writes a durable audit
-- row with action 'retract_symptom'. Without this value in the CHECK the insert
-- is rejected on PostgreSQL -- after the retraction itself has been stored --
-- so the patient is told the withdrawal failed while it has in fact happened.
-- The memory backend enforces no CHECK, which is why only the vocabulary gate
-- (`scripts/check-audit-action-vocabulary.py`) sees it.
--
