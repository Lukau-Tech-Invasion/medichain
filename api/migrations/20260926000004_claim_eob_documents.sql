...
-- =============================================================================
-- A payer's explanation of benefits, filed by an administrator against an
-- insurance claim, readable and downloadable by the claim's patient. The bytes
-- are encrypted in the IPFS document pipeline; this row is the index.
--
-- Until a row exists for a claim, the patient app says "No EOB received yet".
-- There was only an `eob_received` flag and a UI field that was always null;
-- nothing is backfilled because no EOB document ever existed.
-- =============================================================================

CREATE TABLE IF NOT EXISTS claim_eob_documents (
    id              TEXT PRIMARY KEY,
    -- Claims are JSON records in insurance_claims; the claim id is
    -- checked by the handler, which loads the claim before storing anything.
...
CREATE INDEX IF NOT EXISTS idx_claim_eob_documents_claim
    ON claim_eob_documents (claim_id, created_at DESC);

-- =============================================================================
-- access_logs.action: the two EOB acts. Restated in full.
-- =============================================================================
